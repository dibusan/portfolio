...
extension SizedContext on BuildContext {
  ...
  /// Returns if the closest context widget has a larger width
  bool isSmallerWidthThan(double width){
    final RenderBox? renderBox = findRenderObject() as RenderBox?;
    if(renderBox == null) return false;
    return renderBox.size.width < width;
  }

  /// Returns if the closest context widget has a larger height
  bool isSmallerHeightThan(double height){
    final RenderBox? renderBox = findRenderObject() as RenderBox?;
    if(renderBox == null) return false;
    return renderBox.size.width < height;
  }

  double pctHeightFromPx({
    required double pixels,
    double? maxHeight,
  }) {
    final pct = (pixels) / (maxHeight ?? heightPx);
    return pct;
  }

  double pctWidthFromPx({
    required double pixels,
    double? maxWidth
  }) {
    final pct = (pixels) /(maxWidth ?? widthPx);
    return pct;
  }
